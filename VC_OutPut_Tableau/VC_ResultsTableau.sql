...
select  ebe.codpais, 
 ...
 where ebe.aniocampana = vc.aniocampana(+)
   ...
   Delete from dom_virtual_coach.det_vc_dbresults
   where codpais||aniocampana||codebelista in (select codpais||aniocampana||codebelista from #det_vcoutputtableau);
  
   insert into dom_virtual_coach.det_vc_dbresults
   select * from #det_vcoutputtableau;
  
   drop table if EXISTS #det_vcoutputtableau;