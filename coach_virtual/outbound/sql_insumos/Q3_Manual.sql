...
DROP TABLE IF EXISTS #descripcion;
SELECT        b.CODPAIS,
              b.aniocampanaexpo as aniocampana,
              a.codcuc,
              b.codventa,
              case a.codcategoria when '01' then 'FG'
              					when '02' then 'MQ'
              					when '04' then 'CP'
              					when '09' then 'TC'
              					when '10' then 'TF' else a.codcategoria end codcategoria,
              b.descategoria as descripcion
INTO   #descripcion
FROM   fnc_analitico.dwh_dproducto a
inner join lan_virtual_coach.fdettemplates b on a.codcuc = b.codcuc
WHERE  EXISTS
              (      SELECT        *
                     FROM   #PaisCampanas c
                     WHERE  b.codpais = c.codpais AND
                                   b.aniocampanaexpo = c.aniocampana
              )
group by b.codpais,b.aniocampanaexpo,a.codcuc,b.codventa,a.codcategoria,b.descategoria;
