...
for table in stgTables:
    try:
        cursor.execute("Delete sbx_temp.TP"+ table + ";")
...
for table in stgTables:
    try:
        cursor.execute("copy sbx_temp.TP" + table + " from 's3://" + sys.argv[8] + "/" + sys.argv[9] + "/" + table + "/" + table + "' access_key_id '" + sys.argv[6] + "' secret_access_key '" + sys.argv[7] + "' GZIP REMOVEQUOTES ACCEPTINVCHARS delimiter '\t' region 'us-east-1' escape;")
...
try:
		cursor.execute("select CodPais,AnioCampana into #TempDelete "
				   "from sbx_temp.tptmp_arp_corp_det_tactica_condicion group by CodPais,AnioCampana;" )
		...
		cursor.execute("INSERT INTO fnc_analitico.corp_det_tactica_condicion "
				"SELECT * FROM sbx_temp.tptmp_arp_corp_det_tactica_condicion ;")
...
try:
		cursor.execute("select CodPais,AnioCampana into #TempDelete "
				   "from sbx_temp.tptmp_arp_corp_det_tactica_oferta group by CodPais,AnioCampana;" )
		...
		cursor.execute("INSERT INTO fnc_analitico.corp_det_tactica_oferta "
				"SELECT * FROM sbx_temp.tptmp_arp_corp_det_tactica_oferta ;")
		cursor.execute("DROP TABLE #TempDelete;")
except Exception as error:
			sys.exit('4: ' + repr(error))			
...
try:
		cursor.execute("select CodPais,AnioCampana into #TempDelete "
				   "from sbx_temp.tptmp_arp_corp_mae_tactica_condicion group by CodPais,AnioCampana;" )
		...
		cursor.execute("INSERT INTO fnc_analitico.corp_mae_tactica_condicion "
				"SELECT * FROM sbx_temp.tptmp_arp_corp_mae_tactica_condicion ;")
		cursor.execute("DROP TABLE #TempDelete;")
except Exception as error:
			sys.exit('4: ' + repr(error))			
...
try:
		cursor.execute("select CodPais,AnioCampana into #TempDelete "
				   "from sbx_temp.tptmp_arp_corp_mae_tactica_vinculada group by CodPais,AnioCampana;" )
		...
		cursor.execute("INSERT INTO fnc_analitico.corp_mae_tactica_vinculada "
				"SELECT * FROM sbx_temp.tptmp_arp_corp_mae_tactica_vinculada ;")
		cursor.execute("DROP TABLE #TempDelete;")
except Exception as error:
			sys.exit('4: ' + repr(error))			
