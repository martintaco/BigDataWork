...
for table in stgTables:
	try:
		...
		for pais in paises:
			cursor.execute("copy lan_dm_analitico." + table + " from 's3://" + sys.argv[8] + "/" + sys.argv[9] + "/" + table + "/" + pais + "/" + pais + '_' + str(start_time.year)+str_moth + str_day + '.csv' + "' access_key_id '" + sys.argv[6] + "' secret_access_key '" + sys.argv[7] + "' emptyasnull NULL AS 'NULL' IGNOREHEADER AS 1 REMOVEQUOTES ACCEPTINVCHARS delimiter '\t' region 'us-east-1' escape;")
...
try:
		cursor.execute("select planvisitaID,pais into #TempDelete "
				   "from lan_dm_analitico.tmp_arp_det_planrutardd group by planvisitaID,pais;" )
		...
		cursor.execute("INSERT INTO dom_digital.det_planrutardd "
				"SELECT * FROM lan_dm_analitico.tmp_arp_det_planrutardd ;")
...
try:
		cursor.execute("select ID,Pais into #TempDelete "
				   "from lan_dm_analitico.tmp_arp_det_cabplanrutardd group by ID,Pais;" )
		...
		cursor.execute("INSERT INTO dom_digital.det_cabplanrutardd "
				"SELECT * FROM lan_dm_analitico.tmp_arp_det_cabplanrutardd ;")
		cursor.execute("DROP TABLE #TempDelete;")
except Exception as error:
			sys.exit('4: ' + repr(error))
