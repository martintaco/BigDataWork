--Genera el listado de activas según la campaña
...
--Genera las campañas a considerar, por regla de negocio deben
--considerarse las 3 ultimas campañas
drop table if exists #campanasvalidas;
create table #campanasvalidas as 
select distinct codpais, aniocampana from fnc_analitico.dwh_fstaebecam
where aniocampana between f_calculaaniocampana('CO','202001',-2) and '202001';

--Genera el detalle de las ventas a nivel de consultora para las 3 ultimas campañas	
...
--Crea y pobla la tabla según el orden de priorizacion que se va considerar para la recomendación.
...
--Crea y pobla una tabla que valida que la marca que se va recomendar se venda en el pais
--Ej. En Panama no se vende Esika por eso no figura en esta tabla 
drop table if exists #marcasPorPais;
create temporary table #marcasPorPais
(codpais varchar(2),
marca varchar(4)
);

insert into #marcasPorPais values ('BO','CZ');
insert into #marcasPorPais values ('BO','EK');
insert into #marcasPorPais values ('BO','LB');
insert into #marcasPorPais values ('CL','CZ');
insert into #marcasPorPais values ('CL','EK');
insert into #marcasPorPais values ('CL','LB');
insert into #marcasPorPais values ('CO','CZ');
insert into #marcasPorPais values ('CO','EK');
insert into #marcasPorPais values ('CO','LB');
insert into #marcasPorPais values ('CR','CZ');
insert into #marcasPorPais values ('CR','EK');
insert into #marcasPorPais values ('CR','LB');
insert into #marcasPorPais values ('DO','CZ');
insert into #marcasPorPais values ('DO','EK');
insert into #marcasPorPais values ('DO','LB');
insert into #marcasPorPais values ('EC','CZ');
insert into #marcasPorPais values ('EC','EK');
insert into #marcasPorPais values ('EC','LB');
insert into #marcasPorPais values ('GT','CZ');
insert into #marcasPorPais values ('GT','EK');
insert into #marcasPorPais values ('GT','LB');
insert into #marcasPorPais values ('MX','CZ');
insert into #marcasPorPais values ('MX','EK');
insert into #marcasPorPais values ('MX','LB');
insert into #marcasPorPais values ('PA','CZ');
insert into #marcasPorPais values ('PA','LB');
insert into #marcasPorPais values ('PE','CZ');
insert into #marcasPorPais values ('PE','EK');
insert into #marcasPorPais values ('PE','LB');
insert into #marcasPorPais values ('SV','CZ');
insert into #marcasPorPais values ('SV','EK');
insert into #marcasPorPais values ('SV','LB');


--Genera una tabla a nivel de detalle aniocampana, codpais, codebelista que indica por cada una de las marcas-categorias
--cuanto se vendió en moneda y cantidad
...
--Agrupa la vta de las 3 campañas. Nivel de detalle codpais, codebelista
...
/*
* Genera 3 flags:
* 1. FlagMultimarcaPrincipal.- Indica si la consultora es multimarca y a la vez multicategoría,
*    es decir vende todas las combinaciones.
* 2. FlagMultiCategoria.- Indica si la consultora es multicategoria, es decir vende las 3 categorias pero no las 3 marcas.
* 3. FlagMultiMarca.- Indica si la consultora es multimarca, es decir vende las 3 marcas pero no las 3 categorias.
*/
...
--Genera una tabla que tiene todas las consultoras como multimarca, es un cross join de todas las consultoras
--con todas las combinaciones posibles de marcas-categorias.
drop table if exists #baseideal0;
create table #baseideal0 as
select t.codpais, t.codebelista, m.marcacategoria, m.marca, m.categoria
from #total t
cross join #marcacatprioriza m;

--Aqui hace el ajuste con la tabla #marcasporpais para considerar solo las marcas validas por pais.
drop table if exists #baseideal;
create table #baseideal as 
select a.codpais, a.codebelista, a.marcacategoria, a.marca, a.categoria
from #baseideal0 a 
inner join #marcasporpais b on a.codpais = b.codpais and a.marca = b.marca;

--Genera una tabla que tiene lo real, por cada consultora lo que realmente vende en marca-categoria.
drop table if exists #basereal0;
create table #basereal0 as
select codpais, codebelista, (marcafinal + categoriafinal) as marcacategoria, marcafinal as marca, categoriafinal as categoria 
from #uuvta a
group by codpais, codebelista, marcafinal + categoriafinal,marcafinal, categoriafinal;

--Aqui hace el ajuste para solo considerar las marcas-categorias de la tabla de priorizacion
--porque hay categorias como 'NC' que no deben considerarse.
...
/*
 * Para este requerimiento se han identificado que una consultora puede caer en 4 escenarios
 * Caso1:Es multimarca y es multicategoria 
 * Caso2:Es multimarca y no es multicategoria
 * Caso3:No es multimarca y es multicategoria
 * Caso4:No es multimarca y no es multicategoria
 * 
 * Cada subgrupo se evaluará por separado porque cada grupo tiene un 
 * flujo espeficico de qué es lo que se le debe recomendar.
 */
...
drop table if exists #multimarca;
create table #multimarca as
select '202001' as aniocampana,a.codpais, a.codebelista, flagmultimarcaprincipal as FlagMultimarca
, b.marcacategoria as RecomendacionMultimarca1 
from #resumen a
