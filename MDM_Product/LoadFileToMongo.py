...
for obj in s3.list_objects(Bucket = S3_BUCKET_NAME_LOADED, Prefix = S3_PATH_FILES)['Contents']:
    ...
    if obj['Key'] != S3_PATH_FILES:
        ...
        df = pd.read_csv(StringIO(csv_string), sep='\t',dtype = str,header = 0, names = ['aniocampana','codcanalventa','codcatalogo','codestrategia',
                                                                                          'codtipooferta','codventa','descatalogo','destipooferta',
                                                                                          'nropagina','numoferta','precionormalmn','preciooferta',
                                                                                          'preciovtapropuestomn','codtipocatalogo','desargventa','desexposicion',
                                                                                          'desladopag','destipocatalogo','desubicacioncatalogo','fotomodelo',
                                                                                          'fotoproducto','nropaginas','paginacatalogo','desobservaciones',
                                                                                          'vehiculoventa','codpais','codsap','codtipomedioventa',
                                                                                          'demandaanormalplan','desestrategia','destipodiagramacion',
                                                                                          'factorrepeticion','flagdiscover','flagestadisticable','flagproductosebe                                                                                          ','indcuadre','indpadre','precionormaldolplan','precionormalmnplan',                                                                                              'precioofertadolplan','precioofertamnplan','factorcuadre','flagdigitable',
                                                                                          'nrogrupo','nropaginadigital'])
