...
def CalculaAnioCampana(anioCampana, delta):
	...
	#executor python3
	#python3 /home/belcorpuser/scripts/FVirtual-DProyectada/proceso_dwh_FV.py 10.12.2.26 5439 analitico belcorpuser B3lc0rp18$ AKIAJK6A3CSH7NDH2TWA WenXCHfRDCitIqeXvGtG+2puDFXbzRN33W2Y/zfU FVirtual-DProyectada FVirtual.sql FaltanteProl_20190410.csv
...
stgTables = ['temp_faltante_virtual']


for table in stgTables:
    try:
        cursor.execute("DELETE FROM dom_forecast." + table + ";")
...
for table in stgTables:
    try:		
        cursor.execute("copy dom_forecast." + table + " from 's3://belc-bigdata-landing-dlk-prd/datalake/input/sap/FaltanteVirtual/" + filename + "' access_key_id '" + sys.argv[6] + "' secret_access_key '" + sys.argv[7] + "' ACCEPTANYDATE DATEFORMAT 'auto' ACCEPTINVCHARS IGNOREHEADER AS 1 delimiter ',' region 'us-east-1' escape;")
